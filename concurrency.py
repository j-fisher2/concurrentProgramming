...
def main_sequential(list1, list2, list3):  # Main wrapper for sequential example
  s = time.perf_counter()
  # your code goes here
  a1=cal_average(list1)
  a2=cal_average(list2)
  a3=cal_average(list3)
...
def main_threading(list1, list2, list3):  # Main wrapper for threading example
  ...
  threads=[]
  for list in lists:
    t=threading.Thread(target=cal_average,args=(list,))
    threads.append(t)
    t.start()
...
def main_multiprocessing(list1, list2, list3):  # Main wrapper for multiprocessing example
  ...
  # your code goes here
  lists=[list1,list2,list3]
  processes=[Process(target=cal_average,args=(l,)) for l in lists]
  for p in processes:
    p.start()
  for p in processes:
    p.join()
